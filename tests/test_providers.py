...
def test_providers_dict_has_all() -> None:
    ...
    assert {"openrouter", "openai", "azure", "anthropic", "local"}.issubset(set(PROVIDERS.keys()))
...
def test_anthropic_constructs_with_env_key() -> None:
    # Force an empty credential file so the env-var fallback is exercised
    # (without this, a local ~/.karna/credentials/anthropic.token.json wins).
    with (
        patch.dict(os.environ, {"ANTHROPIC_API_KEY": "sk-ant-test"}),
        patch("karna.providers.base.BaseProvider._load_credential", return_value={}),
    ):
