...
async def test_send_message_to_completed_agent(tmp_path, monkeypatch):
    ...
    # Provider returns different content each call
    call_count = 0
...
async def test_task_tool_background_returns_agent_id(tmp_path, monkeypatch):
    ...
    assert "bg_task" in parsed["agent_name"] or "bg-task" in parsed["agent_name"]
...
@pytest.mark.asyncio
async def test_task_tool_default_is_background(tmp_path, monkeypatch):
    """Default run_in_background=True behavior."""
    monkeypatch.chdir(tmp_path)
    provider = _SlowProvider(content="default bg", delay=0.1)
    ...
    # Should be JSON (background mode)
    parsed = json.loads(result)
    assert parsed["status"] == "started"
...
@pytest.mark.asyncio
async def test_task_tool_send_message_action(tmp_path, monkeypatch):
    """TaskTool send_message action routes to manager."""
    monkeypatch.chdir(tmp_path)
    ...
    # First create a foreground agent
