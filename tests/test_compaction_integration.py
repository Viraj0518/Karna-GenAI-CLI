...
- Token estimation via the agent loop's internal estimator
"""
...
from karna.compaction.compactor import CompactionError, Compactor
...
class TestAutoCompactionTrigger:
    ...
    async def test_compaction_fires_at_80_percent(self):
        ...
        # Each message ~400 chars = ~100 tokens. 12 messages = ~1200 tokens.
        # Context window = 1000 tokens. 80% threshold = 800 tokens.
        # So 12 messages should trigger compaction.
        messages = [Message(role="user" if i % 2 == 0 else "assistant", content="x" * 400) for i in range(12)]
        ...
        assert len(conv.messages) < original_count + 1  # +1 for final assistant

        # The summary message should be present
        contents = " ".join(m.content for m in conv.messages)
        assert "Compacted summary" in contents or "COMPACTED" in contents
    ...
    async def test_no_compaction_below_threshold(self):
        ...
        # 3 short messages = ~15 tokens, well below 80% of 100000
        messages = [
    ...
    async def test_compaction_in_sync_loop(self):
        ...
        messages = [Message(role="user" if i % 2 == 0 else "assistant", content="y" * 400) for i in range(12)]
        ...
        assert len(conv.messages) < 13  # 12 original + 1 final
        contents = " ".join(m.content for m in conv.messages)
        assert "SYNC COMPACTED" in contents or "Compacted summary" in contents
...
class TestCircuitBreaker:
    ...
    async def test_circuit_breaker_trips_after_3_failures(self):
        """After 3 compaction failures, the compactor's circuit breaker trips."""
        ...
        # Build large conversation (needs enough msgs for head + tail)
        messages = [Message(role="user" if i % 2 == 0 else "assistant", content="z" * 400) for i in range(15)]
        conv = Conversation(messages=messages)

        # The Compactor wrapping auto_compact raises CompactionError after
        # 3 internal attempts; each call to compact() that raises counts as
        # a circuit-breaker trip.
        with pytest.raises(CompactionError):
            await compactor.compact(conv, context_window=1000)
    ...
    async def test_circuit_breaker_skips_compaction_in_loop(self):
        ...
        messages = [Message(role="user" if i % 2 == 0 else "assistant", content="w" * 400) for i in range(12)]
...
class TestPreservedTail:
    ...
    async def test_tail_messages_preserved(self):
        """The last tail_turns_to_keep messages must survive compaction.

        The default tail is 8 turns in the auto_compact function.
        """
        # Create 20 messages with identifiable content
        tail_size = 8  # default tail_turns_to_keep in auto_compact
        total = 20
        messages = []
        ...
        # The last tail_size messages should be present
        preserved_contents = [m.content for m in result.messages[-tail_size:]]
        for i in range(total - tail_size, total):
    ...
    @pytest.mark.asyncio
    async def test_head_messages_preserved(self):
        """Head messages (first 2 turns by default) are always preserved."""
        messages = [
            Message(role="system", content="You are a helpful assistant."),
            Message(role="user", content="Initial question about architecture."),
        ]
        for i in range(18):
            role = "user" if i % 2 == 0 else "assistant"
        ...
        # Head messages (system + first user) should be preserved
        assert result.messages[0].role == "system"
        assert result.messages[0].content == "You are a helpful assistant."
        assert result.messages[1].content == "Initial question about architecture."
...
class TestCompactSlashCommand:
    ...
    def test_compact_command_with_provider(self):
        ...
        # Build a conversation large enough to compact (> 10 msgs = > 6 threshold)
        messages = [Message(role="user" if i % 2 == 0 else "assistant", content="x" * 400) for i in range(14)]
        ...
        with mock_patch("karna.tui.slash.get_provider", return_value=mock_provider):
            with mock_patch("karna.tui.slash.resolve_model", return_value=("mock", "mock-model")):
...
# ======================================================================= #
#  Tests: token estimation in agent loop
# ======================================================================= #


class TestTokenEstimation:
    """Verify the agent loop's internal token estimation."""
...
# ======================================================================= #
#  Tests: compactor should_compact
# ======================================================================= #
