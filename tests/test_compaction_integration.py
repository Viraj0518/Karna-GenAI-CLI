...
class TestAutoCompactionTrigger:
    ...
    async def test_compaction_fires_at_80_percent(self):
        ...
        # Need enough messages for head(2) + tail(8) + middle(>0) = at least 11.
        # 20 messages x ~400 chars = ~2000 tokens.  Context window = 2000.
        messages = [
            Message(
    ...
    async def test_compaction_in_sync_loop(self):
        ...
        # Need 20+ messages so head(2) + tail(8) leaves a middle to compact.
        messages = [
            Message(
                role="user" if i % 2 == 0 else "assistant",
                content="y" * 400,
        ...
        contents = " ".join(m.content for m in conv.messages)
        assert "Compacted summary" in contents or "SYNC COMPACTED" in contents
...
class TestCircuitBreaker:
    ...
    async def test_circuit_breaker_trips_after_3_failures(self):
        ...
        # Build large conversation (needs enough messages for head+tail split)
        messages = [
            Message(
                role="user" if i % 2 == 0 else "assistant",
                content="z" * 400,
        ...
        # The compact call should raise CompactionError after 3 internal retries,
        # which trips the circuit breaker.
        from karna.compaction.compactor import CompactionError
...
class TestPreservedTail:
    ...
    async def test_tail_messages_preserved(self):
        """The tail messages must survive compaction."""
        ...
        # The Compactor on main uses auto_compact with default tail_turns_to_keep=8
        compactor = Compactor(summary_provider, threshold=0.50)
        conv = Conversation(messages=messages)

        result = await compactor.compact(conv, context_window=1000)

        # The last 8 messages (default tail_turns_to_keep) should be present
        tail_contents = [m.content for m in result.messages[-8:]]
        for i in range(7, 15):
            expected_prefix = f"Message-{i}"
            assert any(expected_prefix in c for c in tail_contents), f"Message-{i} should have been preserved in tail"
