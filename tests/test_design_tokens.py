...
def test_palette_values_are_valid_hex() -> None:
    ...
    all_colors = [
        dt.COLORS.bg.subtle, dt.COLORS.bg.raised,
        dt.COLORS.border.subtle, dt.COLORS.border.accent,
        dt.COLORS.text.primary, dt.COLORS.text.secondary,
        dt.COLORS.text.tertiary, dt.COLORS.text.disabled,
        dt.COLORS.accent.brand, dt.COLORS.accent.hover,
        dt.COLORS.accent.cyan, dt.COLORS.accent.success,
        dt.COLORS.accent.warning, dt.COLORS.accent.danger,
...
def test_semantic_roles_present() -> None:
    required = {
        ...
        "accent.brand", "accent.cyan", "accent.success", "accent.warning",
        "accent.danger", "accent.thinking",
        "role.user", "role.assistant", "role.thinking",
        "tool.name", "tool.args", "tool.pending",
        "tool.running", "tool.ok", "tool.err",
        "meta", "divider", "prompt",
    }
...
def test_spacing_is_integer_column_counts() -> None:
    ...
    assert all(isinstance(v, int) for v in
               (dt.SPACING.xs, dt.SPACING.sm, dt.SPACING.md,
                dt.SPACING.lg, dt.SPACING.xl))


def test_typography_roles_complete() -> None:
    for role in ("heading.1", "heading.2", "body", "caption", "micro",
                 "code", "emphasis", "meta"):
...
def test_required_icons_exist() -> None:
    ascii_set = icons_mod.IconSet(force_ascii=True)
    required = [
        "tool_bash", "tool_read", "tool_write", "tool_edit", "tool_grep",
        "tool_glob", "tool_git", "tool_web", "tool_mcp", "tool_task",
        "tool_monitor",
        "user", "assistant", "thinking", "sparkle",
        "success", "error", "pending", "chevron_right",
    ]
...
def test_new_semantic_styles_exist() -> None:
    for attr in (
        ...
        "THINKING", "TOOL_NAME", "TOOL_OK", "TOOL_ERR",
        "DIVIDER", "META", "HEADING_1", "EMPHASIS",
    ):
...
def test_karna_theme_is_rich_theme_with_all_roles() -> None:
    ...
    required_styles = {
        ...
        "tool.status.ok", "tool.status.err",
        "meta", "divider",
        # legacy aliases must still be present
        "panel.border", "banner.border", "prompt",
    }
