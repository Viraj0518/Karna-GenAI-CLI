...
_PATTERNS: Final[tuple[tuple[str, re.Pattern[str]], ...]] = (
    ...
    ("ignore_previous_instructions",
     re.compile(r"\b(?:ignore|disregard|forget)\b[^.!?]{0,40}\b(?:previous|prior|above|system)\b[^.!?]{0,40}\b(?:instructions?|prompts?|rules?|directives?)", _FLAGS)),
    ("ignore_all_instructions",
     re.compile(r"\b(?:ignore|disregard|forget)\b\s+(?:all|any|every)\s+(?:instructions?|prompts?|rules?|guidelines?)", _FLAGS)),
    ("reveal_system_prompt",
     re.compile(r"\b(?:reveal|show|print|output|display|dump|reproduce|repeat|tell\s+me|give\s+me|share)\b[^.!?]{0,40}\b(?:system\s*prompt|original\s+instructions?|hidden\s+instructions?|prompt\s+template)", _FLAGS)),

    # Role hijacking
    ("role_hijack_you_are_now",
     re.compile(r"\b(?:you\s+are\s+now|from\s+now\s+on\s+you\s+are|act\s+as\s+a?|pretend\s+(?:to\s+be|you\s+are)|assume\s+the\s+role\s+of|roleplay\s+as)\b[^.!?]{0,60}", _FLAGS)),
    ("role_hijack_dan",
    ...
    ("delim_im_start",
     re.compile(r"<\|(?:im_start|im_end|system|user|assistant|endoftext)\|>", _FLAGS)),
    ("delim_chatml",
     re.compile(r"<\|?\s*role\s*=\s*[\"']?(?:system|assistant)[\"']?\s*\|?>", _FLAGS)),

    # Tool-call / function-call spoofing
    ("tool_call_spoof_json",
     re.compile(r"\"(?:name|tool|function)\"\s*:\s*\"(?:bash|write|edit|read|execute|shell|eval|exec|os\.system|subprocess)\"", _FLAGS)),
    ("tool_call_spoof_xml",
     re.compile(r"<\s*(?:tool_?call|function_?call|invoke|use_tool)\b[^>]*>", _FLAGS)),
    ("tool_call_spoof_shell",
     re.compile(r"(?:```|`)\s*(?:bash|sh|shell|powershell|cmd)\s*\n[^`]{0,500}(?:rm\s+-rf|curl[^`]*\|\s*(?:sh|bash)|wget[^`]*\|\s*(?:sh|bash)|eval\b|exec\b)", _FLAGS)),

    # Output manipulation
    ("base64_blob",
     re.compile(r"\b(?:base64|b64|atob)\b\s*[:\(][^)]{0,20}[A-Za-z0-9+/]{100,}={0,2}", _FLAGS)),
    ("repeat_forever",
     re.compile(r"\b(?:repeat|output|print|loop)\b\s+(?:the\s+word\s+)?[\"']?\w+[\"']?\s+(?:forever|infinitely|a\s+thousand\s+times|until\s+token\s+limit)", _FLAGS)),
    ...
    ("urgent_override_auth",
     ...
     re.compile(r"\b(?:as\s+(?:the\s+)?|i\s+am\s+(?:the\s+)?|this\s+is\s+(?:the\s+)?)(?:admin|root|superuser|owner|developer|anthropic|openai|google)\b[^.!?]{0,30}\b(?:override|unlock|authoriz\w+|grant\s+access|bypass)\b", _FLAGS)),
    ("grandma_trick",
     re.compile(r"\b(?:my\s+grandma|my\s+grandmother|my\s+late\s+\w+)\b[^.!?]{0,80}\b(?:used\s+to|always|would)\s+(?:tell|read|recite)", _FLAGS)),

    # Data exfiltration markers
    ("exfil_encode_url",
     re.compile(r"\b(?:encode|embed|exfil\w*)\b[^.!?]{0,30}\b(?:into|in|to)\s+(?:the\s+)?(?:url|image|link|href|filename)\b", _FLAGS)),
    ("exfil_send_to_attacker",
     re.compile(r"\b(?:send|post|upload|exfil\w*|leak)\b[^.!?]{0,30}\b(?:https?://(?!localhost|127\.0\.|0\.0\.)|attacker\.com|evil\.com|malicious\.)", _FLAGS)),
)
