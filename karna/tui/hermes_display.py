"""CLI presentation — spinner, glyph faces, tool preview formatting.

Port of Hermes ``agent/display.py`` into Nellie. Mirrors the upstream
module 1:1; the only intentional deltas are:

* Brand color → Nellie's ``BRAND_BLUE`` (``#3C73BD``) from
  :mod:`karna.tui.themes`.
* Glyph vocabulary — tool-call ``●``, tool-result ``⎿``,
  assistant label ``◆ nellie``, thinking ``✦ Thinking…``.
* Skin lookups (``hermes_cli.skin_engine.get_active_skin``) are shimmed:
  imports are wrapped in ``try / except ImportError`` and fall through
  to the semantic tokens in :mod:`karna.tui.design_tokens`.

Every function signature and class name is preserved so the sibling
``karna.tui.hermes_repl`` module can import the same surface as Hermes's
``cli.py``.
...
try:
    # Hermes ships ``safe_json_loads`` in ``utils.py``; Nellie does not have a
    # drop-in equivalent, so we define a compatible helper inline below.
    from utils import safe_json_loads  # type: ignore[import-not-found]
except ImportError:  # pragma: no cover - fallback for Nellie's layout
    import json as _json
    from typing import Any as _Any

    def safe_json_loads(text: str, default: _Any = None) -> _Any:
        """Parse JSON, returning *default* on any parse error.

        Mirrors ``utils.safe_json_loads`` from Hermes; we inline it here so
        ``karna.tui.hermes_display`` has no hard dependency on a shared utils
        module that Nellie doesn't ship.
        """
        try:
...
# =========================================================================
# Reasoning/thinking tag stripping (ported from Hermes cli.py so the
# sibling repl module can import it from one canonical place).
# =========================================================================
...
# =========================================================================
# Nellie-specific glyph vocabulary (branding only — every other behaviour
# mirrors Hermes).  Kept at module scope so hermes_repl.py can import and
# use them directly.
# =========================================================================

NELLIE_TOOL_CALL_GLYPH = "\u25cf"          # ● tool call
NELLIE_TOOL_RESULT_GLYPH = "\u23bf"        # ⎿ tool result
NELLIE_ASSISTANT_LABEL = "\u25c6 nellie"   # ◆ nellie
NELLIE_THINKING_LABEL = "\u2726 Thinking\u2026"  # ✦ Thinking…
...
def _hex_to_ansi_fg(hex_str: str, fallback_rgb: tuple[int, int, int]) -> str:
    """Lift a ``#RRGGBB`` hex string into an ANSI 24-bit foreground escape."""
    if hex_str and len(hex_str) == 7 and hex_str[0] == "#":
...
def _diff_ansi() -> dict[str, str]:
    ...
    try:
        from hermes_cli.skin_engine import get_active_skin  # type: ignore[import-not-found]
        ...
        # Nellie fallback path — pull the equivalents out of design tokens.
        try:
            from karna.tui.design_tokens import SEMANTIC

            dim = _hex_to_ansi_fg(SEMANTIC.get("text.tertiary", ""), (150, 150, 150))
            file_c = _hex_to_ansi_fg(SEMANTIC.get("accent.brand", ""), (180, 160, 255))
            hunk = _hex_to_ansi_fg(SEMANTIC.get("border.subtle", ""), (120, 120, 140))

            err_h = SEMANTIC.get("accent.danger", "")
            ok_h = SEMANTIC.get("accent.success", "")
            if err_h and len(err_h) == 7 and err_h[0] == "#":
                er, eg, eb = int(err_h[1:3], 16), int(err_h[3:5], 16), int(err_h[5:7], 16)
                minus = (
                    f"\033[38;2;255;255;255;48;2;"
                    f"{max(er//2,20)};{max(eg//4,10)};{max(eb//4,10)}m"
                )
            if ok_h and len(ok_h) == 7 and ok_h[0] == "#":
                or_, og, ob = int(ok_h[1:3], 16), int(ok_h[3:5], 16), int(ok_h[5:7], 16)
                plus = (
                    f"\033[38;2;255;255;255;48;2;"
                    f"{max(or_//4,10)};{max(og//2,20)};{max(ob//4,10)}m"
                )
        except Exception:
            pass
    except Exception:
...
def _get_skin():
    ...
    Under Nellie the ``hermes_cli.skin_engine`` module does not exist, so
    the ``ImportError`` falls through and callers pick up their semantic
    token fallbacks.
    """
    try:
        from hermes_cli.skin_engine import get_active_skin  # type: ignore[import-not-found]
...
def get_skin_tool_prefix() -> str:
    ...
    Nellie fallback uses ``NELLIE_TOOL_RESULT_GLYPH`` (⎿) per the branding
    spec — matches the ``tool.result`` L-branch glyph expected by the
    integration tests.
    """
    skin = _get_skin()
    if skin:
        try:
            ...
            pass
    return NELLIE_TOOL_RESULT_GLYPH
...
def _resolve_skill_manage_paths(args: dict) -> list[Path]:
    ...
    try:
        from tools.skill_manager_tool import _find_skill, _resolve_skill_dir  # type: ignore[import-not-found]
    except ImportError:
        # Nellie does not ship skill_manager_tool — return no targets so
        # the surrounding diff pipeline short-circuits cleanly.
        return []
...
def _emit_inline_diff(diff_text: str, print_fn) -> bool:
    ...
    if print_fn is None or not diff_text:
        return False
    try:
        print_fn(f"  {NELLIE_TOOL_RESULT_GLYPH} review diff")
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in the inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    """Animated spinner with glyph faces for CLI feedback during tool execution."""
